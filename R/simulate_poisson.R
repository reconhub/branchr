...
#' \itemize{
#'
#' \item z_sim: the final outbreak sizes for each simulated outbreaks. Note that for outbreaks that are not extinct
#' 'final' by the end of the simulation, z_sim correspond to the cumulative incidence by time t_max.
#'
#' \item tata: il est caca
#'
#' }#'observed incidence (per generation) accounting for under-reporting.
#' If some simulated outbreak are not extinct by the end of the simulation, a warning is displayed.
#'
#' @examples
#'
#' x <- simulate_poisson(.8, 2)
#' x
#'
simulate_poisson <- function(R,n,s,rho,t_max) {
	...
	if (length(n)!=1 | n=0 | !is.integer(n)) warning('n must be a positive integer')
	if (length(t_max)!=1 | t_max=0 | !is.integer(T_max)) warning('t_max must be a positive integer')

	# declare incidence matrices
  N <- matrix(NA,n,t.max)
  N_obs_full <- matrix(NA,n,t.max)
